...
## Summary
- 10306 nodes · 14675 edges · 1060 communities (770 shown, 290 thin omitted)
...
Cohesion: 0.04
Nodes (62): confirmedResponse(), PayoutRow, POST(), requestSchema, POST(), replySchema, DELETE(), PUT() (+54 more)
...
Cohesion: 0.08
Nodes (59): 管理者 RBAC 権限チェック (authorizeAdminPermission), src/app/api/items/[id]/route.ts, src/app/api/items/route.ts, src/app/api/news/route.ts, Cache-Control: private, no-store, PublicItemGrid（catalog variant）, SectionTitle（home variant 専用）, item 一覧 セキュリティレビュー (+51 more)

### Community 20 - "createServiceRoleClient"
Cohesion: 0.03
Nodes (73): applyCsrfProtection(), GET(), POST(), AUDIT_LOGS_SAFE_COLUMNS, GET(), createTemplateSchema, GET(), POST() (+65 more)
...
### Community 25 - "orders/[id]/page.tsx"
Cohesion: 0.12
...
### Community 33 - "src/app/search/page.tsx"
Cohesion: 0.17
Nodes (15): アフォーダンス, CT-1 全画面黒バー明滅＋"BARS REVEAL"デバッグ文言, combobox ARIA パターン（aria-expanded/activedescendant）, SearchPageClient.tsx, ドハティのしきい値（応答フィードバック）, 空状態と回復導線, ID-4 カート追加成功のインライン通知とカート導線, L-4 0件時に「すべて見る」ボタンを併設 (+7 more)
...
Cohesion: 0.04
Nodes (74): src/app/api/admin/news/[id]/route.ts, src/app/api/admin/news/route.ts, src/app/api/admin/items/route.ts, src/app/api/admin/looks/[id]/route.ts, src/app/api/admin/looks/route.ts, Audit logging, audit_logs, src/app/api/cart/[id]/route.ts (+66 more)

### Community 57 - "look/server.ts"
Cohesion: 0.06
Nodes (46): GET(), PATCH(), updateStatusSchema, formatCurrency(), formatOrderDateTime(), GET(), mapPaymentMethodLabel(), NO_STORE_HEADERS (+38 more)
...
### Community 74 - "PublicStockistGrid.tsx"
Cohesion: 0.09
...
Cohesion: 0.05
Nodes (78): Cache-Control: no-store（機微応答のキャッシュ抑止）, double-submit Cookie CSRF トークン (sb-csrf-token / x-csrf-token), honeypot フィールド (website), isSameOriginRequest（Origin/Referer 検証）, MFA / TOTP 登録・検証, user_id / session_id 所有権スコープ, proxy.ts STATE_CHANGING_PATH_PREFIXES 同一オリジン強制, status='published' ガード（API＋DB 両層） (+70 more)

### Community 80 - "accounting-sync.ts"
Cohesion: 0.22
...
### Community 137 - "BottomNavigation.tsx"
Cohesion: 0.36
...
### Community 151 - "complete-route.test.ts"
Cohesion: 0.22
...
Cohesion: 0.03
Nodes (90): AdminItemEditPage(), ItemForm(), ItemFormProps, CATEGORIES, Category, ColorInput, ColorPresetResponse, ItemFormValues (+82 more)
...
Cohesion: 0.10
Nodes (33): aria-current 付きパンくず, brand.md（ブランドコンセプト/ペルソナ）, breadcrumb（HOME起点）パターン, ItemDetailClient.tsx, LookImageGallery.tsx, クロスセル導線, デザイン4原則: 対比, エラー防止 (+25 more)
...
Cohesion: 0.03
Nodes (99): Page(), AccordionItem, AccordionProps, ActionSheet(), ActionSheetAction, ActionSheetProps, UIActionSheetShape, Avatar() (+91 more)
...
### Community 1018 - "デザイン4原則（近接・整列・反復・対比）"
Cohesion: 0.25
Nodes (8): A-4 同一画像の反復で情報量ゼロ, AC-7 フィードバックが操作地点から遠い, VF-2 OTP入力が LOGIN のセグメント式と不一致, CT-3 行小計が無く数量変更の金額影響が不明, デザイン4原則（近接・整列・反復・対比）, I-5 件数表示・適用中フィルタチップが無い, OD-6 金額内訳が無く合計のみ, システム状態の可視性
...
### Community 1032 - "src/app/auth/callback/page.tsx"
Cohesion: 0.32
...
### Community 1034 - "Additional Resources"
Cohesion: 0.50
...
### Community 1042 - "File Organization and Naming"
Cohesion: 0.50
...
Cohesion: 0.06
Nodes (45): AccountingStripeClient, OrderRow, POST(), requestSchema, StripeBackfillResponse, orderIdSchema, OrderLookupRow, POST() (+37 more)
...
## Knowledge Gaps
- **4930 isolated node(s):** `ItemPageSearchParams`, `LookPageSearchParams`, `NewsPageSearchParams`, `metadata`, `dynamic` (+4925 more)
