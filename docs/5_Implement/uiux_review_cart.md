...
- **CT-2**: ♡/削除ボタンに `min-h-11 min-w-11` + `aria-label` を付与。Stepper を `size="4xs"` → `size="xs"` に拡大。
- **CT-3**: 各行に「単価」+「行小計（単価×数量）」を表示。数量変更で即時更新。
- **CT-4 / CT-5**: 合計を「合計（税込）」に変更し、「価格は税込表示です。送料・最終金額はご購入手続きで確定します。」の補記を追加。
- **CT-6**: ページのエラーを `red-50/red-300` → `text-red-600 + border-black/15 bg-black/2`（最小限の赤）、同期警告の `amber-*` → モノトーン（`text-[#474747]` + 罫線 + `ri-error-warning-line` アイコン）へ。`role` も付与。
