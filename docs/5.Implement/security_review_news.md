...
| [migrations/013_create_news_images_bucket.sql](../../migrations/013_create_news_images_bucket.sql) | news-images バケットを public=true で作っており、private 状態の記事でも画像がアップロード時点で恒久公開 URL 化される | 未公開記事用の画像は private バケットに分離し、公開済み記事のみ署名 URL か公開用パスへ昇格させる | 対応済み |
| [src/app/api/admin/news/route.ts](../../src/app/api/admin/news/route.ts) | News 記事の作成・更新・公開状態変更・削除、および画像アップロードに対して監査記録がない | 管理系 news API で actor、resource_id、status 変更前後、storage object path、結果を audit_logs に記録する | 対応済み |