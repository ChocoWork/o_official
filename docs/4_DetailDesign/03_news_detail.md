...
|--------|----------|--------|----------------|----------|--------------|
| FR-NEWS-DETAIL-001 | 公開済みニュース記事の詳細ページを表示する | IMPL-NEWS-DETAIL-001 | `src/app/news/[id]/page.tsx`, `src/features/news/services/public.ts`, `src/lib/storage/news-images.ts` | RSC で `getPublishedNewsDetailById` を呼び出し、private な `news-images` バケットの画像はサーバ側で署名 URL に変換する。存在しない場合は `notFound()` | 済 |
