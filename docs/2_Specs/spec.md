...
- **FR-LOGIN-002** `MUST`: Google OAuth によるソーシャルログインボタンを表示する。
  - レビュー: `LoginModal` 内に Google OAuth ボタンが存在 ✅。
...
**概要**: ログインユーザーのプロフィール・注文履歴・配送先住所の管理。

- **FR-ACCOUNT-001** `MUST`: プロフィールタブ・注文履歴タブ・配送先住所タブを持つタブ型レイアウトを提供する。
  - レビュー: `TabSegmentControl` で profile / orders / address の3タブを実装 ✅。`?tab=` URL パラメータとも連動 ✅。要件を満たしている。
- **FR-ACCOUNT-002** `MUST`: プロフィールタブではメールアドレス（読み取り専用）・氏名・電話番号を表示・編集できる。
  - レビュー: メールアドレスが `"demo@gmail.com"` にハードコードされており、ログインユーザーの実際のメールアドレスを表示していない ❌。氏名・電話番号は `/api/profile` GET/POST/DELETE で取得・保存・削除 ✅。
...
- **FR-ACCOUNT-004** `MUST`: 配送先住所タブでは郵便番号・都道府県・市区町村・番地を入力・保存できる。郵便番号入力時に住所自動補完を実行する。
  - レビュー: フォーム UI ✅、郵便番号補完は `https://zipcloud.ibsnet.co.jp/api/search?zipcode=...` を直接呼び出し（キャッシュ API 未使用 ❌）。`<form>` に `onSubmit` がなく住所保存のバックエンド実装が未完了 ❌。
- **FR-ACCOUNT-005** `MUST`: 注文履歴タブでは過去の注文を一覧表示し、注文詳細への遷移を提供する。
  - レビュー: ORD-ABC123 / ORD-DEF456 / ORD-GHI789 などのハードコードダミーデータと readdy.ai 画像を表示 ❌。実際の注文データを `/api/orders` 等から取得するロジックが未実装。
- **FR-ACCOUNT-006** `SHOULD`: 顧客プロフィールの保存項目を拡張し、フリガナ・住所を含めた完全な顧客情報を管理できるようにする。
  - レビュー: 現状は氏名・電話番号のみ保存。フリガナ・住所フィールドは未実装 ❌。
...
- **WF-ACCOUNT-001**: 目的 — 顧客の自己管理（注文確認、アドレス管理、サイズプロファイル）。
- **WF-ACCOUNT-002**: 主なセクション — 注文履歴（ステータスリンク）、アカウント情報、アドレス帳、ウィッシュリスト、クーポン履歴。
...
- **WF-CHECKOUT-B01**: ステップ 1 — 配送先情報（住所入力、住所帳から選択）。
...
- **WF-ACCOUNT-B01**: 注文履歴、注文詳細、返品申請、アカウント設定、サイズプロファイル。
