...
| FREQ-230 | コスト＆利益ビューの「コスト入力」タブの新規経費フォームで、「カテゴリ」を「勘定科目」に、自由入力の「項目」を「摘要（使用用途）」の固定プルダウンに変更し、さらに「取引先」プルダウン（登録済みから選択＋新規登録）を追加すること | FREQ-230-REQ-01 | 新規経費フォームおよび経費一覧テーブルの見出しラベル「カテゴリ」を「勘定科目」に変更すること（データ列 category は変更しない） | FREQ-230-AC-01 | mobile（390px）/ tablet（768px）/ desktop（1280px）で、コスト入力タブのフォームと経費一覧に「勘定科目」ラベルが表示され、「カテゴリ」が表示されないこと | FREQ-230-REQ-02 | 自由入力だった「項目」を「摘要（使用用途）」の固定プルダウン（生地・材料仕入／サンプル制作／縫製外注／副資材・附属購入／広告出稿／撮影・制作／展示会・イベント／梱包・発送／打合せ・交通／システム・ツール利用料／その他）に変更すること。ユーザーによる選択肢追加は不可とすること | FREQ-230-AC-02 | 同3ビューポートで「摘要（使用用途）」がプルダウン（combobox）で表示され、定義済みの選択肢（例「展示会・イベント」）を選んで経費を保存すると一覧・仕訳帳に反映されること | FREQ-230-REQ-03 | 「取引先」プルダウンを追加すること。全シーズン共通（グローバル）の取引先マスタ admin_finance_partners（migration 065、name UNIQUE、RLS有効・service role経由のみ、admin.finance.read/manage 権限で制御）を新設し、登録済み取引先を選択できること。経費テーブル admin_finance_expenses に partner 列（text NOT NULL DEFAULT ''）を追加し、選択した取引先を保存すること | FREQ-230-AC-03 | 同3ビューポートで「取引先」プルダウンに登録済み取引先と「＋新規登録」項目が表示され、取引先を選んで保存した経費の取引先が一覧・仕訳帳に反映されること | FREQ-230-REQ-04 | 取引先プルダウンで「＋新規登録」を選ぶと入力欄が現れ、取引先名を入力して登録すると /api/admin/kpi/cost-profit へ operation=partner.create でPOSTされ、成功後にマスタへ追加されて選択済みになること | FREQ-230-AC-04 | 同3ビューポートで「＋新規登録」選択→取引先名入力→登録すると、その取引先がプルダウンの選択肢（option）に追加されること | FREQ-230-AC-05 | 同3ビューポートで横方向のページスクロールが発生しないこと |
| FREQ-231 | コスト入力タブの新規経費フォームのプルダウン（勘定科目・摘要・取引先・支払い方法）を、既存の黄金比ベースのUIコンポーネント SingleSelect で実装すること（ネイティブ select を置換） | FREQ-231-REQ-01 | 各プルダウンを @/components/ui/SingleSelect の SingleSelect（variant="dropdown"・block・size="md"）で描画し、他の管理フォーム（商品・ニュース等）と同じ黄金比デザインのトリガー＋ポータル型リストにすること。値の選択は onValueChange で受け取り、既存の保存・取引先新規登録フローを維持すること | FREQ-231-AC-01 | mobile（390px）/ tablet（768px）/ desktop（1280px）で、勘定科目・摘要（使用用途）・取引先・支払い方法が SingleSelect のトリガー（button、シェブロン付き）として表示され、押すと選択肢リスト（role=option）が開くこと | FREQ-231-AC-02 | 同3ビューポートで、摘要のリストから「展示会・イベント」を選び金額を入力して保存でき、取引先のリストから「＋ 新規登録」を選んで取引先を登録すると選択肢に追加されること | FREQ-231-AC-03 | 同3ビューポートで横方向のページスクロールが発生しないこと |
| FREQ-232 | コスト入力タブで、勘定科目・摘要・金額・支払い方法・メモの5項目を「テンプレート」として保存・管理し、テンプレートを選択して経費フォームへ一括反映できるようにすること | FREQ-232-REQ-01 | 全シーズン共通（グローバル）のテンプレートマスタ admin_finance_expense_templates（migration 066、name UNIQUE、勘定科目/摘要/金額/支払い方法/メモを保持、RLS有効・service role経由のみ、admin.finance.read/manage で制御）を新設すること。作成・削除は新規経費フォーム上の「テンプレート」プルダウンで完結させ（専用管理画面は設けない）、/api/admin/kpi/cost-profit の operation=template.create（同名は上書き）・template.delete で永続化すること | FREQ-232-AC-01 | mobile（390px）/ tablet（768px）/ desktop（1280px）で、新規経費フォーム先頭に「テンプレート」プルダウン（SingleSelect）があり、選択肢に保存済みテンプレートと「＋ 現在の入力を保存」が並ぶこと | FREQ-232-REQ-02 | 「＋ 現在の入力を保存」を選ぶとテンプレート名の入力欄が現れ、初期値を「摘要 / 金額」（例「縫製外注 / ¥50,000」）とし、保存前に名前を変更できること。保存すると現在の5項目がその名前で保存され、選択肢に追加されること | FREQ-232-AC-02 | 同3ビューポートで、摘要「縫製外注」・金額50000を入力して「＋ 現在の入力を保存」を選ぶと、名前欄の初期値が「縫製外注 / ¥50,000」になり、名前を変更して保存すると「テンプレートを保存しました。」が表示され選択肢に追加されること | FREQ-232-REQ-03 | 保存済みテンプレートを選ぶと勘定科目・摘要・金額・支払い方法・メモがフォームへ反映されること（日付・取引先は変更しない）。選択中テンプレートは横の削除ボタンで削除できること | FREQ-232-AC-03 | 同3ビューポートで、別の摘要に変えた後に保存済みテンプレートを選び直すと摘要・金額が保存時の値に戻ること。削除ボタンでテンプレートを削除でき「テンプレートを削除しました。」が表示されること | FREQ-232-AC-04 | 同3ビューポートで横方向のページスクロールが発生しないこと |
| FREQ-233 | コスト＆利益ビューの「コスト入力」タブを「収支入力」に改称し、支出だけでなく収入も管理できるようにすること。収入は売上高として財務サマリー・損益計算書へ自動反映すること。あわせてテンプレートのプルダウンを他の入力欄と同じ全幅に揃えること | FREQ-233-REQ-01 | サブタブのラベルを「コスト入力」から「収支入力」に変更すること（タブキー expenses は変更しない）。新規登録フォーム先頭に「種別（支出/収入）」トグルを置き、1フォームで切替できること。種別に応じて勘定科目・摘要の選択肢を切替（支出＝経費科目、収入＝売上高/雑収入等）、支払い方法ラベルを収入時は「入金方法」にすること | FREQ-233-AC-01 | mobile（390px）/ tablet（768px）/ desktop（1280px）で、サブタブに「収支入力」が表示され「コスト入力」が表示されないこと。収支入力タブに「支出一覧」と「収入一覧」が別々に表示されること | FREQ-233-REQ-02 | 収支は entry_type（expense/income、migration 067）で区別して admin_finance_expenses に保存し、GETは支出（expenses）と収入（incomes）に分けて返すこと。テンプレートも entry_type を持ち、選択時に種別ごとフォームへ反映すること | FREQ-233-AC-02 | 同3ビューポートで、種別を「収入」にして摘要「オンライン販売」・金額120000で「収入をSupabaseへ保存」すると、entryType=income のPOSTが送られ「収入一覧（1件）」に反映されること | FREQ-233-REQ-03 | 登録した収入の合計を売上高として財務サマリー・損益計算書・税務レポートに反映すること（収入未登録なら従来の財務前提の手入力値を使用）。テンプレートのプルダウンを他の入力欄と同じ全幅（block）で表示すること | FREQ-233-AC-03 | 同3ビューポートで、収入120000登録後に財務サマリーの売上（見込み）が「¥120,000」を表示すること | FREQ-233-AC-04 | 同3ビューポートで横方向のページスクロールが発生しないこと |
