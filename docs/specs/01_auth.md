...
## パスワードハッシュ方針（追記 — Argon2 への移行）

- **保護の方向性**: パスワードは一方向ハッシュのみで保存し、平文を保持しません。既存実装で用いられている `scrypt` / `bcrypt` 等の既存ハッシュとの互換性を保ちつつ、新規および再ハッシュは `Argon2id` を採用します。

- **採用理由**: Argon2id はパスワードストレージにおける現代的な推奨アルゴリズムであり、メモリ・時間コストの調整による耐攻撃性（GPU/ASIC 対策）を提供します。

- **実装（サーバ）**:
  - 新規ユーザ作成・パスワード更新時は `Argon2id` でハッシュ化して `password_hash` カラムへ保存します。
  - 既存ユーザの検証では、保存されているハッシュの形式に応じて以下を行います:
    1. 保存値が Argon2 フォーマット（`$argon2` で始まる）なら `argon2.verify()` を使用して検証する。
    2. それ以外（例: `scrypt$...`, `$2a$` など）の場合は既存の照合ロジックで検証し、認証成功時に Argon2 で再ハッシュして更新する（オンログイン再ハッシュ）。

- **Argon2 パラメータ（現時点の推奨値、環境に応じ要調整）**:
  - Variant: Argon2id
  - Time cost: 3
  - Memory cost: 4096 KiB (4 MiB)
  - Parallelism: 1
  - 出力長: ライブラリ既定（例: 32 バイト、ライブラリがエンコードした `$argon2id$...` 文字列を保存）

- **DB スキーマの軽微変更（移行補助）**:
  - 既存行がどのアルゴリズムでハッシュされているかを管理するため、`password_hash_algo` カラムを追加する移行を推奨します（既存レコードは `scrypt` 等既存値をデフォルトにセット）。これは運用と監査のためであり、必須ではありませんが、移行の可視化とバルク調査に有用です。

- **オンデマンド移行戦略（推奨）**:
  1. アプリ側で **互換性レイヤ** を導入（現在の実装では `src/lib/hash.ts` が該当）: 保存されているハッシュの形式を判定して適切な検証を行い、認証成功時に Argon2 で再ハッシュして保存する。
  2. 併用期間中は `password_hash` 形式を見て検証を分岐させる。既存ユーザは次回ログイン時に順次 Argon2 化される。
  3. 監査・運用目的で `password_hash_algo` を追加してマイグレーション進捗を追跡する（オプション）。

- **強制バルク移行オプション（運用上の検討）**:
  - 平文がないため直接バルクで Argon2 に変換することは不可能です。代替案:
    - パスワード再設定フローを強制（メールで全ユーザに再設定リンクを送付）
    - 一定期間内に再設定が行われないアカウントはロックする等の運用ルール
    - または、パスワード有効なユーザに対して次回ログイン時に自動再ハッシュを実施する（推奨）

- **備考（DB 互換性）**:
  - `password_hash` カラムの型は `text` で十分です。Argon2 の文字列表現（`$argon2id$v=19$m=...,t=...,p=...$...$...`）は既存の `text` 長に収まります。
  - 既存マイグレーションで `password_hash` を `text NOT NULL` としている場合、追加のスキーマ変更は不要です（ただし `password_hash_algo` を追加する場合は新規マイグレーションを追加してください）。


### セキュリティ注意点
- Argon2 パラメータは本番環境の CPU / メモリ制約に合わせて調整してください。CI / テスト環境ではコストを下げるなど環境差を設けることを推奨します。
- 秘匿キーやハッシュ化パラメータの変更は運用チームと調整の上、ロールアウト計画を作成してください。
