...
| 借入・発生累計 | 対象科目の貸方額合計 |
| 返済・支払済み | 対象科目の借方額合計 |
| 借入・事業主資金の返済残高 | 借入・投入累計から返済・引出済みを差し引いた金額 |
| その他の支払債務の残高 | 選択年度の総勘定元帳における期末残高 |
...
- 借入・事業主資金の返済残高は、全期間の借入・投入累計から返済・引出済みを差し引いて算出する。事業主借も同じ計算規則とする。
- その他の支払債務は、過去の仕訳から特定できる相手先別差引残高を現在残高へ割り当てる。
- その他の支払債務における相手先別差引残高の合計と総勘定元帳残高との差額は、科目ごとに「繰越・相手先未設定」として表示する。期首残高の相手先は推測しない。
...
| 借入・投入累計 | 貸方計上の累計 |
| 返済・引出済み | 借方計上の累計 |
| 返済残高 | 借入・投入累計から返済・引出済みを差し引いた金額 |
| 最終入出金日 | 最新仕訳日。繰越のみの場合は「—」 |

- 通常の借入科目では返済残高を返済判断に使う。
- 事業主借では「事業主資金の返済残高」として扱い、「法的な返済義務額ではなく、決算時に元入金へ振り替えられます」と注記する。
- パネル上部に借入・投入累計、返済・引出済み、返済残高の合計を表示する。
...
- その他の支払債務で相手先別残高合計と元帳残高が一致しない場合、差額を表示して帳簿確認を促す。
...
- その他の支払債務の期首繰越残高を「繰越・相手先未設定」へ計上する。
- 相手先未入力、完済、過払い、繰越のみを処理できる。
- 借入・事業主資金の返済残高が借入・投入累計から返済・引出済みを差し引いた値と一致する。
- その他の支払債務の残高合計が対象科目の総勘定元帳残高と一致する。

### 6.2 コンポーネントテスト

- 財務概要に2表と各列見出しが表示される。
- 借入累計、返済済み、返済残高、最終入出金日が表示される。
- 事業主借の説明と空状態が表示される。
...
- 選択年度末時点で、借入先ごとの借入・投入累計、返済・引出済み、返済残高が確認できる。
- その他の支払債務が借入・事業主資金と分離されている。
- 事業主借を法的な返済義務額と誤認しない表示になっている。
- その他の支払債務は相手先未設定や期首繰越を欠落させず、元帳残高と一致する。
