...
- 通常の借入・債務は、過去の仕訳から特定できる相手先別差引残高を現在残高へ割り当てる。
- 事業主借は決算時に元入金へ振り替えられるため、受入・引出累計は全期間、現在残高は選択年度内の仕訳と期首残高から算出する。
- 相手先別差引残高の合計と総勘定元帳残高との差額は、科目ごとに「繰越・相手先未設定」として表示する。期首残高の相手先は推測しない。
