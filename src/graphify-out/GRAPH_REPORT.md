...
- `DELETE()` --calls--> `syncCategoryQuery()`  [INFERRED]
  app\api\wishlist\[id]\route.ts → features\news\components\PublicNewsGrid.tsx
- `DELETE()` --calls--> `handleClear()`  [INFERRED]
  app\api\wishlist\[id]\route.ts → features\search\components\SearchPageClient.tsx
