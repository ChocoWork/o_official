...
export function MetaKpiConnection({
  season,
  onSynced,
}: MetaKpiConnectionProps) {
  const [data, setData] = useState<ConnectionData | null>(null);
  const [isWorking, setIsWorking] = useState(false);
  const [message, setMessage] = useState<string | null>(null);

  const fetchStatus = useCallback(async () => {
    const response = await clientFetch("/api/admin/kpi/meta", {
      cache: "no-store",
    });
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(
          "同期に失敗しました。Metaの権限とアクセストークンを確認してください。",
        );
  ...
  const lastSynced = data.connection?.last_synced_at
    ? new Intl.DateTimeFormat("ja-JP", {
        ...
        timeStyle: "short",
        timeZone: "Asia/Tokyo",
      }).format(new Date(data.connection.last_synced_at))
    : "未同期";

  return (
    <div
      className="rounded-lg border border-[#d4d4d4] bg-[#fafafa] p-3"
      aria-label="Instagram・Meta広告連携"
    >
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          {data.connected ? (
            ...
            <Button
              ...
              onClick={() =>
                window.location.assign("/api/admin/kpi/meta/connect")
              }
            >
              Metaと接続
            </Button>
