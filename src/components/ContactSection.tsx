...
export default function ContactSection() {
  ...
  const fetchThreads = useCallback(async () => {
    ...
    try {
      ...
      const res = await clientFetch(`/api/admin/contact?${params.toString()}`, {
        cache: "no-store",
      });
  ...
  const fetchTemplates = useCallback(async () => {
    try {
      const res = await clientFetch("/api/admin/contact/templates", {
        cache: "no-store",
      });
      if (!res.ok) return;
      const json = await res.json();
  ...
  const relevantTemplates = useMemo(() => {
    ...
    return templates.filter(
      (template) =>
        ...
        template.category === detail.inquiry_type,
    );
  }, [templates, detail]);
              ...
              {threads.map((thread) => (
                <li key={thread.id}>
                  <button
                    ...
                    className={`w-full text-left border p-3 transition-colors ${selectedId === thread.id ? "border-black bg-black/5" : "border-black/10 hover:bg-black/2"}`}
                  >
            ...
            <div className="space-y-4">
              <Card size="md" className="space-y-2">
                ...
                <dl className="grid grid-cols-1 gap-x-6 gap-y-1 text-sm text-[#474747] font-acumin sm:grid-cols-2">
                  ...
                  <div>
                    <dt className="inline font-medium">メール：</dt>
                    <dd className="inline">{detail.email}</dd>
                  </div>
                  <div>
                    <dt className="inline font-medium">種別：</dt>
                    <dd className="inline">
                      {TYPE_LABELS[detail.inquiry_type]}
                    </dd>
                  </div>
                  <div>
                    <dt className="inline font-medium">注文番号：</dt>
                    <dd className="inline">{detail.orderNumber ?? "—"}</dd>
                  </div>
                  <div>
                    <dt className="inline font-medium">受信日時：</dt>
                    <dd className="inline">
                ...
                <div className="flex flex-wrap items-end gap-2">
                  <div className="w-full sm:w-64">
                    <SingleSelect
                      ...
                      options={relevantTemplates.map((template) => ({
                        value: template.id,
...
function TemplateManager({ templates, onChanged }: TemplateManagerProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = editingId
        ? await clientFetch(`/api/admin/contact/templates/${editingId}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: payload,
          })
        : await clientFetch("/api/admin/contact/templates", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: payload,
          });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await clientFetch(`/api/admin/contact/templates/${id}`, {
        method: "DELETE",
      });
      if (!res.ok) throw new Error(`Failed: ${res.status}`);
                ...
                <p className="truncate text-xs text-[#474747] font-acumin">
                  {
                    TEMPLATE_CATEGORY_OPTIONS.find(
                      (option) =>
                        option.value === (template.category ?? "general"),
                    )?.label
                  }
                </p>
              </div>
      ...
      <div className="space-y-2 border-t border-black/10 pt-3">
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button
            ...
            onClick={handleSave}
            disabled={
              isSaving || title.trim().length === 0 || body.trim().length === 0
            }
          >
            {isSaving ? "保存中..." : "保存"}
          </Button>
