...
// タブ列端から内側に離し、各タブの字間を tracking-[0.2em] → tracking-widest に縮小する。
...
test.describe("FR-SEARCH-016 タブの左右余白と字間", () => {
  test("AC-01 モバイルで先頭/末尾タブがタブ列端より内側に入る", async ({
    page,
  }) => {
    ...
    expect(measured.firstTabLeft - measured.tablistLeft).toBeGreaterThanOrEqual(
      8,
    );
    expect(
      measured.tablistRight - measured.lastTabRight,
    ).toBeGreaterThanOrEqual(8);
    expect(measured.hasHorizontalScroll).toBe(false);
  });

  for (const vp of VIEWPORTS) {
    test(`AC-02/AC-03 ${vp.name} 字間 0.1em と下線の端揃え`, async ({
      page,
    }) => {
      ...
      for (const tab of tabs) {
        ...
        expect(
          Math.abs(tab.letterSpacing - tab.expectedSpacing),
