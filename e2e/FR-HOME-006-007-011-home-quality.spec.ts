...
test.describe("FR-HOME-006 / 007 / 011 home quality checks", () => {
  test("sets home metadata, prioritizes hero image, and keeps accessible structure", async ({
    page,
  }) => {
    await page.goto("/");
    ...
    await expect(
      page.locator('section.relative div[aria-hidden="true"].bg-linear-to-b'),
    ).toBeVisible();

    const aboutSection = page.locator("#about");
    await expect(aboutSection).toBeVisible();

    const aboutTextColor = await aboutSection
      .locator("p")
      .first()
      .evaluate((element) => {
