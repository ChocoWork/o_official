...
/** 隣り合うセグメント線が隙間なく並んでいることを確認する */
async function expectSegmentsHaveNoGap(
  page: Page,
  indicatorTestId: string,
): Promise<void> {
  const lines = page
    .getByTestId(indicatorTestId)
    .getByRole("tab")
    .locator("span");
  ...
  for (let i = 0; i < count - 1; i += 1) {
    const current = await lines.nth(i).boundingBox();
    const next = await lines.nth(i + 1).boundingBox();
    expect(current).not.toBeNull();
    expect(next).not.toBeNull();
    expect(current!.x + current!.width).toBeCloseTo(next!.x, 1);
  }
}
...
test.describe("FR-ITEM-DETAIL-038 商品画像カルーセルのインジケータと送りボタン", () => {
  test.describe("mobile 390x844", () => {
    ...
    test("FREQ-271-AC-05: 線と線の間に隙間がない", async ({ page }) => {
      await openItemDetail(page);
      await expectSegmentsHaveNoGap(page, "item-detail-carousel-indicator");
    });
  });
  ...
  test.describe("desktop 1280x900", () => {
    ...
    test("FREQ-271-AC-05: 線と線の間に隙間がない", async ({ page }) => {
      await openItemDetail(page);
      await expectSegmentsHaveNoGap(page, "item-detail-main-image-indicator");
    });
  });
});
