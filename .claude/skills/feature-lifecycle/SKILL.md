...
| **Developer** | 設計・実装・ユニットテスト・指摘反映 | 全ツール + agent |
| **Reviewer** | 仕様レビュー・設計レビュー・コードレビュー | read / search / execute |
...
## Phase 2: 設計・実装計画（🧑‍💼 設計レビューあり）

| Step | 担当 | 内容 |
|------|------|------|
| 2.1 | Architect | 設計・実装計画を作成し、仕様書 Issue にコメントとして投稿し、（`communication-protocol.md` のフォーマットに従う） |
| 2.2 | Reviewer | 設計・実装計画をレビュー → APPROVE / REQUEST CHANGES |
| 2.3 | Architect | REQUEST CHANGES の場合、指摘を反映して 2.2 へ戻る |

- 設計レビューループ上限: **2 回**（超過時は人間にエスカレーション）

### 設計・実装計画の必須成果物

Phase 3 に進むためには、以下が設計・実装計画に含まれていること:
...
| 3.2 | Developer | 実装・静的解析（`npm run lint`）・ビルドの成功確認（`npm run build`） |
| 3.3 | Developer | 影響を受けるユニットテスト（`docs/6_Test` ）を作成・更新し、ユニットテスト実行（`npm run test`） |
| 3.4 | Orchestrator | 実装完了を検知 |
| 3.5 | Documentation | 実装差分を解析し、影響を受ける `docs/` / `README.md` / `docs/6_Test/` を更新し同 PR にコミット |
| 3.6 | Orchestrator | PR を Ready for Review に変更 |
| 3.7 | Reviewer | コードレビュー（コード + docs 更新）→ APPROVE / REQUEST CHANGES |
| 3.8 | Developer / Documentation | REQUEST CHANGES の場合、指摘を反映して 3.7 へ戻る |
| 3.9 | Tester | E2E テスト実行 → PASS / FAIL |
| 3.10 | Developer | FAIL の場合、バグ修正して 3.9 へ戻る |
| 3.11 | Architect | 完成判断（docs 最新化も判定基準に含む）→ COMPLETE / INCOMPLETE |
| 3.12 | Developer / Documentation | INCOMPLETE の場合、改善して 3.8 へ戻る |
| 3.13 | 🧑‍💼 **人間** | **マージの最終承認**（ここで人間が判断を挟む） |
| 3.14 | Orchestrator | 承認後 → Squash Merge、ブランチ削除 |
