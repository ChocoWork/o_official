...
```powershell
# Windows / PowerShell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .claude/skills/security-check/scripts/audit.ps1 --files-only src/lib

# Pythonを直接実行
python .claude/skills/security-check/scripts/audit.py --files-only src/lib
```

```bash
# Git Bash / Linux / macOS（既存互換）
bash .claude/skills/security-check/scripts/audit.sh --files-only src/lib
```
