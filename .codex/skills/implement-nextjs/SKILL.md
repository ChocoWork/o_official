...
description: Next.js 16 App Router実装スキル。Next.js、App Router、Server Components、Client Components、Route Handlers、Server Actions、metadata、画像最適化、キャッシュ戦略、Turbopack、TypeScriptの相談や実装依頼で必ず使用すること。特に、params/searchParamsの非同期化対応、use cacheとCache Components、updateTag/revalidateTag/refreshの使い分け、proxy.tsへの移行、React 19.2連携が絡む場合はこのスキルを優先する。
...
Next.js 16系のApp Router前提で、実装からレビューまで一貫して品質を引き上げるためのスキル。

## When to Use This Skill

- Next.js機能の新規実装や改修を行うとき
- App Router配下に`page.tsx`、`layout.tsx`、`loading.tsx`、`error.tsx`、`route.ts`を追加・変更するとき
- ルーティング、データ取得、キャッシュ、Server Actions、Route Handlersを扱うとき
- `params` / `searchParams` / `cookies()` / `headers()`の扱いで設計判断が必要なとき
- SEO、metadata、画像最適化、パフォーマンス最適化を同時に満たす必要があるとき

## Core Principles

1. App Routerを標準とする。新規機能は`app/`配下で設計する。
2. Server Componentを既定とし、インタラクションが必要な最小範囲だけClient Componentにする。
3. Next.js 16の非同期Request APIを前提に設計する。
4. キャッシュは意図を明示する。`use cache`、`cacheTag`、`cacheLife`、`updateTag`、`revalidateTag`を目的別に使う。
5. 型安全と境界管理を優先する。UI層、データ取得層、更新処理層を分離する。
6. まず動くコードではなく、運用で壊れにくいコードを目指す。

## Next.js 16 Critical Rules

### 1) Async request props and APIs

- `params` と `searchParams` はPromiseとして扱う。
- `cookies()`、`headers()`、`draftMode()`は`await`して使う。
- 同期アクセス前提の実装は移行時の不具合源になるため避ける。
...
### 2) Cache Components first

- `cacheComponents: true`を有効化してから`use cache`を使う。
- データレベル・UIレベルどちらにキャッシュを置くかを明示する。
- リクエストごとの最新値が必要な箇所は`use cache`を使わず、`Suspense`でストリーミングする。
...
### 3) Revalidation API selection

- `updateTag(tag)`:
  Server Action内でread-your-writesが必要なときに使う。
- `revalidateTag(tag, 'max')`:
  SWR前提でタグを無効化したいときに使う。
- `revalidateTag(tag)`単引数は非推奨。必ず第2引数を指定する。
- `refresh()`:
  Server Actionから未キャッシュデータを再取得させたいときに使う。

### 4) Bundler and config updates

- Turbopackは既定バンドラ。特別な理由がなければそのまま使う。
- `experimental.turbo`ではなく`turbopack`設定を使う。

### 5) Network boundary naming

- 新規実装は`proxy.ts`を優先する。
- 既存の`middleware.ts`は段階的移行を計画する。

## Server/Client Composition Guidelines

- Server Component側で`next/dynamic` + `{ ssr: false }`は使わない。
- Client側ロジックは`'use client'`付きコンポーネントに集約し、Server Componentから通常importする。
- ブラウザAPI、イベント、state、effectsが必要な箇所だけClientへ分離する。

## Data Fetching and Mutation Patterns

### Read pattern

1. 取得関数をサーバー側ユーティリティに分離する。
2. fetchのキャッシュ方針を都度明示する。
3. 例外時のフォールバックを`error.tsx`または境界で受ける。

### Mutation pattern

1. Server Actionで入力検証する。
2. 永続化を実行する。
3. `updateTag`または`revalidateTag`で整合性を回復する。
4. 必要に応じて`redirect`や`refresh`でUI反映する。

## Route and File Conventions Checklist

- `page.tsx`: ルート公開の入口
- `layout.tsx`: 共通レイアウト
- `loading.tsx`: セグメント単位のローディング
- `error.tsx`: セグメント単位のエラー境界
- `not-found.tsx`: 404表示
- `route.ts`: 外部公開が必要なAPIエンドポイント
- `default.tsx`: Parallel Routesで必要なスロットの既定ビュー

## TypeScript Guidance

- `PageProps<'/route/[id]'>`の活用を検討する。
- `searchParams`は`string | string[] | undefined`を前提に扱う。
- APIレスポンスは型定義し、`any`を常用しない。
- `Promise`型のpropsを受ける関数で`await`漏れをなくす。

## Performance and SEO Defaults

- 画像は`next/image`を使い、`alt`を必須にする。
- フォントは`next/font`でレイアウトレベルに配置する。
- metadataは`layout.tsx`または`page.tsx`で宣言する。
- 大きい非同期UIは`Suspense`で分割し、初期表示を優先する。

## Security Defaults

- 外部入力はServer Action/Route Handlerで検証する。
- 認可判定はサーバー側で実行する。
- シークレットは環境変数で管理する。
- Webhook経由の更新では`revalidateTag(tag, { expire: 0 })`など要件に応じた一貫性戦略を選ぶ。

## Response Contract

このスキルを使って回答・実装するときは、次を含める。

1. 変更対象のファイル構成
2. Server/Client境界の理由
3. キャッシュ方針と無効化方針
4. 主要なTypeScript型
5. エラー処理とフォールバック
6. パフォーマンス配慮点
...
- App Router `page` reference (async params/searchParams): https://nextjs.org/docs/app/api-reference/file-conventions/page
- Cache Components overview: https://nextjs.org/docs/app/getting-started/caching
- `revalidateTag` reference: https://nextjs.org/docs/app/api-reference/functions/revalidateTag
- `updateTag` reference: https://nextjs.org/docs/app/api-reference/functions/updateTag
- Next.js docs index for LLMs: https://nextjs.org/docs/llms.txt
