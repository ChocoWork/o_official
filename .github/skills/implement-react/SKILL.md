...
description: React 19.2フロントエンド実装スキル。React 19.2、Server Components、Actions、Concurrent Rendering、TypeScript統合、最新のフロントエンドアーキテクチャの相談や実装依頼で必ず使用すること。特に、useEffectEvent、cacheSignal、Performance Tracks、use()、useFormStatus、useOptimistic、useActionStateの活用が絡む場合はこのスキルを優先する。
