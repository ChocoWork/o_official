...
- **技術スタック**: Next.js App Router / React 19 / TypeScript / Supabase / Stripe
- **アーキテクチャ**: Next.js App Router を中心とした機能モジュール構造（`app/` + `components/` + `lib/` + `features/` + `hooks/` + `contexts/`）
- **ビルド**: `npm run build`
- **テスト**: `npm test` / `npm run test:e2e` / `npm run lint`
